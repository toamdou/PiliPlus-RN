...
const SameFollowingRow = memo(function SameFollowingRow({
  item,
  index,
  colors,
  T,
}: {
  item: SameFollowingItem;
  index: number;
...
export default function SameFollowingScreen() {
  ...
  const list = usePagedList<SameFollowingItem>({
    enabled: midNum > 0,
    fetchPage: async (page, cancelToken?: NativeRequestCancelToken) => {
      const res = await userApi.sameFollowing({ mid: midNum, pn: page }, cancelToken ? { cancelToken } : undefined);
    ...
    onError: (e) => {
      console.error('sameFollowing error:', e);
      showToast('加载失败，请重试');
    },
  });

  const renderRow = useCallback(
    ({ item, index }: { item: SameFollowingItem; index: number }) => (
      <SameFollowingRow item={item} index={index} colors={colors} T={T} />
    ),
    [colors, T],
  );
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.bg }]}>
      <Stack.Title large>共同关注</Stack.Title>
        ...
        ListEmptyComponent={
          list.loading ? (
            ...
            <View style={styles.emptyWrap}>
              ...
              <Text style={[T.headline, styles.emptyTitle, { color: colors.text }]}>暂无共同关注</Text>
              <Text style={[T.footnote, styles.emptySub, { color: colors.textSecondary }]}>你和 TA 还没有共同关注的用户</Text>
            </View>
          )
        }
      />
